...
TEST(bloom_filter_basic) {
    ...
    CHECK_EQUAL(loaded.lookup("foo"), 1u);
    CHECK_EQUAL(loaded.lookup("bar"), 1u);
    CHECK_EQUAL(loaded.lookup("baz"), 1u);
    CHECK_EQUAL(loaded.lookup(4.2), 1u);
    CHECK_EQUAL(loaded.lookup('c'), 1u);
    CHECK_EQUAL(loaded.lookup(4711ULL), 1u);
    // OK
    // True-negatives
    CHECK_EQUAL(loaded.lookup("qux"), 0u);
    CHECK_EQUAL(loaded.lookup("graunt"), 0u);
    CHECK_EQUAL(loaded.lookup(3.1415), 0u);
}
